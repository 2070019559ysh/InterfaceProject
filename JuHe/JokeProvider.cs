...
namespace InterfaceProject.OtherSDK.JuHe
{
    ...
    public class JokeProvider:IJokeProvider
    {
        ...
        public JuHeResponse<JokesData> SearchTimeJokes(DateTime beforeTime, JuHeTimeFilter timeFilter = JuHeTimeFilter.After, int pageIndex = 1, int pageSize = 20)
        {
            string sort = "asc";
            if (timeFilter == JuHeTimeFilter.Before)//聚合的排序与我们的习惯相反
                sort = "desc";
            long time = TimeHelper.GetTime(beforeTime);
