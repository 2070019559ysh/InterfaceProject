...
namespace InterfaceProject.WxSDK.LinkUp
{
    ...
    public class TemplateMessageLinkUp
    {
        ...
        public WeChatResult SetIndustry(WeChatIndustry industry1, WeChatIndustry industry2)
        {
            ...
            {
                SystemLogHelper.Warn(GetType().FullName, $"设置所属行业SetIndustry，微信服务报错：{weChatResult}");
            }
            return weChatResult;
        }
        ...
        public WeChatResult DeleteTemplate(string templateId)
        {
            ...
            if (weChatResult.errcode != WeChatErrorCode.SUCCESS)
            {
                SystemLogHelper.Warn(GetType().FullName, $"删除公众号指定的模板DeleteTemplate，微信服务报错：{weChatResult}");
            }
            return weChatResult;
        }
