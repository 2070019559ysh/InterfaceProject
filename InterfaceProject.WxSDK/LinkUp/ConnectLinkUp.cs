    ...
    public class ConnectLinkUp:IConnectLinkUp
    {
        ...
        public string GetAccessToken()
        {
            ...
            do
            {
                ...
                {
                    SystemLogHelper.Info(GetType().FullName, "GetAccessToken(),已有线程直接去微信获取AccessToken，在此等待");
                ...
                else//Redis里面的Token已经失效，需要单线程去更新Token
                {
                    ...
                    {
                        SystemLogHelper.Info(GetType().FullName, "获取公众号的AccessToken,GetAccessToken(),直接去微信获取AccessToken");
        ...
        private string GetWeChatAccessToken()
        {
            ...
            {
                SystemLogHelper.Warn(GetType().FullName, $"直接去微信获取AccessToken,GetWeChatAccessToken()，{accessTokenResult}");
            }
            else
            {
        ...
        public WeChatResult<WeChatIPInfo> GetWeChatServerIP()
        {
            ...
            {
                SystemLogHelper.Warn(GetType().FullName, $"获取微信服务器IP地址GetWeChatServerIP()，微信服务报错：{serviceIPResult}");
            }
