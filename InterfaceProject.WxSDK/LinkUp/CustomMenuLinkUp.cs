...
namespace InterfaceProject.WxSDK.LinkUp
{
    ...
    public class CustomMenuLinkUp: ICustomMenuLinkUp
    {
        ...
        public WeChatResult DeleteMenu(string menuId)
        {
            ...
            if (deleteResult.errcode != WeChatErrorCode.SUCCESS)
            {
                SystemLogHelper.Warn(GetType().FullName, $"删除指定的个性化菜单DeleteMenu()，微信服务报错：{deleteResult}");
            }
            return deleteResult;
        }
        ...
        public WeChatResult<OrdinaryMenu> TryMatchMenu(string userId)
        {
            ...
            {
                SystemLogHelper.Warn(GetType().FullName, $"测试个性化菜单匹配结果TryMatchMenu()，微信服务报错：{matchMenuResult}");
            }
